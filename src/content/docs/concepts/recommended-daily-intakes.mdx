...
description: Recommended Daily Intakes (RDI) or Recommended Daily Allowance (RDA) are the amount of each nutrient that the body needs to function properly. These values are set by health authorities and are based on the latest scientific research. The RDI values are different for different nutrients and are based on factors such as age
---
As we saw in the [Nutrients](/concepts/nutrient) guide, nutrients are essential for the body to function properly. The amount of each nutrient that the body needs is called the **Recommended Daily Intake (RDI)** or **Recommended Daily Allowance (RDA)**.
...
Different countries have different guidelines for RDIs. The following table is an example of various RDIs provided by different health authorities in some of the countries:
